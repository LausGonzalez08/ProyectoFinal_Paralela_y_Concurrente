...
class PhotoFilterApp(TkinterDnD.Tk):
    def __init__(self):
        ...
        # Número de hilos para procesamiento (configurable por usuario)
        self.num_threads = 4
        ...
        # Executor para multihilo (se inicializa con num_threads)
    ...
    def create_widgets(self):
        ...
        process_btn = ttk.Button(controls_frame, text="Aplicar filtro", 
                                command=self.start_processing)
        process_btn.pack(side=tk.LEFT, padx=5)
        
        # Botón para abrir carpeta de resultados
        open_folder_btn = ttk.Button(controls_frame, text="Abrir resultados", 
                                    command=self.open_results_folder)
        open_folder_btn.pack(side=tk.LEFT, padx=5)
        
        # Controles de configuración de hilos
        config_frame = ttk.LabelFrame(top_frame, text="Configuración de Hilos")
        config_frame.pack(side=tk.LEFT, padx=5)
        
        # Frame para controles de hilos
        thread_control_frame = ttk.Frame(config_frame)
        thread_control_frame.pack(padx=5, pady=5)
        
        ttk.Label(thread_control_frame, text="Hilos:").pack(side=tk.LEFT, padx=5)
        
        # Spinbox para seleccionar número de hilos (1-16)
        ...
        # Botón para aplicar cambios de hilos
        apply_threads_btn = ttk.Button(
            thread_control_frame, 
            text="Aplicar", 
            command=self.update_threads,
            width=8
        )
        apply_threads_btn.pack(side=tk.LEFT, padx=5)
        
        # Etiqueta para mostrar configuración actual
        self.threads_label = ttk.Label(
            config_frame, 
            text=f"Actual: {self.num_threads} hilos",
            foreground="#2ECC71",
            font=("Helvetica", 9)
        )
        self.threads_label.pack(pady=(0, 5))
        ...
        # Métricas de tiempo y comparación (agregada métrica de hilos)
        time_labels = ["Secuencial (s)", "Paralelo (s)", "Speedup", "Eficiencia", "Hilos usados"]
        for i, name in enumerate(time_labels):
            ...
            # Crear clave única para cada métrica
            key = name.lower().replace(" (s)", "").replace(" ", "_")
    ...
    def update_threads(self):
        ...
        try:
            new_threads = int(self.threads_spinbox.get())
            if 1 <= new_threads <= 16:
                if new_threads != self.num_threads:
                    self.num_threads = new_threads
                    
                    # Recrear el ThreadPoolExecutor con el nuevo número de hilos
                    ...
                    self.metrics_labels["hilos_usados"].config(text=str(self.num_threads))
                    
                    self.log_message(f"✓ Configuración actualizada: {self.num_threads} hilos")
                    
                    # Si hay procesamiento activo, mostrar advertencia
                    if self.currently_processing:
                        self.log_message("⚠ Cambio de hilos aplicado para el próximo procesamiento")
            else:
                messagebox.showwarning(
                    "Valor inválido",
                    "El número de hilos debe estar entre 1 y 16."
                )
                self.threads_spinbox.set(self.num_threads)
        except ValueError:
            messagebox.showwarning(
    ...
    def start_processing(self):
        ...
        self.log_message(f"Número de hilos: {self.num_threads}")
    ...
    def run_multithread_version(self, selected_filter):
        ...
        # Crear semáforo con el número de hilos configurado
        semaphore = threading.Semaphore(self.num_threads)
    ...
    def run_actor_version(self, selected_filter):
        ...
        results, elapsed = process_image_actor(
            ...
            selected_filter,
            self.output_folder,
            num_actors=self.num_threads  # Usar el mismo número que los hilos
        )
    ...
    def finish_processing(self, parallel_strategy):
        ...
        self.after(0, self.log_message, f"Hilos utilizados: {self.num_threads}")
        ...
        self.after(0, lambda: messagebox.showinfo(
            ...
            f"Hilos utilizados: {self.num_threads}\n"
            f"Speedup: {(self.execution_times['Secuencial'] / self.execution_times['Paralelo']):.2f}x\n"
            f"Eficiencia: {((self.execution_times['Secuencial'] / self.execution_times['Paralelo']) / self.num_threads * 100):.1f}%\n\n"
    ...
    def update_comparison_chart(self, parallel_strategy_name):
        ...
        parallel_label = f"Multihilo ({self.num_threads} hilos)" if self.strategy_var.get() == "multithread" else f"Modelo Actor ({self.num_threads} actores)"
        ...
        if times[1] > 0:
            ...
            efficiency = (speedup / self.num_threads) * 100
            self.ax.text(0.5, 0.90, f'Eficiencia: {efficiency:.1f}% ({self.num_threads} hilos)', 
